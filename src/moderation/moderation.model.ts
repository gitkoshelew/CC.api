...
export class Moderation extends Model<Moderation, ModerationCreationAttrs> {
  ...
  @ApiProperty({example: 'rewrite', description: 'enum status (review/rewrite/change/block/accept)'})
  @Column({ type: DataType.ENUM, values: ['review', 'change', 'block', 'rewrite', 'accept'] })
  status: ModerationStatus;

  //@ApiProperty({example: '[questionId (2) ]', description: 'property to understand for what question relates this moderation field'})
  @HasOne(() => Question)
  question: Question;
}
