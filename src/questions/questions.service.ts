...
export class QuestionsService {
  ...
  async getQuestionById(id: number) {
    return await this.questionRepository.findOne({ where: { id }, include: { all: true} });
  }
  ...
  async addModerationToQuestion(dto: AddModerationToQuestionDto){
    ...
    const moderation = await this.moderationRepository.getModerationById(dto.moderationId)
